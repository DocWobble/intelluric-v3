(() => {
  ...
  function renderSamples(serviceId, serviceLabel) {
    ...
    grid.innerHTML = samples.map((sample, index) => {
      ...
      return `<a class="sample-card" href="/sample-work/${serviceId}/${slugify(title)}">
  ...
  function selectService(card, announce = true) {
    ...
    if (announce) {
      document.dispatchEvent(new CustomEvent('intelluric:service-selected', {
        detail: { service: card.dataset.service, label: card.querySelector('b').textContent.trim() }
      }));
    }
  }
  ...
  document.querySelectorAll('a[href]').forEach(link => link.addEventListener('click', () => {
    document.dispatchEvent(new CustomEvent('intelluric:navigate', {
      detail: { href: link.getAttribute('href'), label: link.textContent.trim() }
    }));
  }));
})();
